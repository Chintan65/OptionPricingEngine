...
void Backtester::run(const std::string& csvPath, double strike, const std::string& expiryDate, bool isCall, bool isBloomberg) {
    ...
    Logger::instance().log(LogLevel::INFO, "Running Interest-Bearing Backtest: " + engine_->getName());
    ...
    while(std::getline(file, line)) {
        ...
        try {
            ...
            if (!isFirstTrade && lastT > T) {
                double dt = lastT - T;
                // Cash grows at the risk-free rate over the time step dt
            ...
            if (isFirstTrade) {
                cash = price; // Initial premium received
                isFirstTrade = false;
            }
